...
effects = {
    ...
    "create_building": "Эффект, позволяющий создать здание. Значение это объект из двух полей: city_id и building_id с указанием на _id города и _id здания соответственно.",
    "change_population": "Эффект, позволяющий изменить население в городе. Значение является объектом из полей city_id (_id города) и population (число, на которое меняется население)."
}
...
def on_effect(effect, value, player, country):
    if effect == "money":
        ...
        city = next((city for city in get_cities(player, 0) if str(city["_id"]) == value["city_id"]), None)
        b = next((b for b in city["buildings"] if b["id"] == value["building_id"]), None)
        db.cities.update_one({"_id": ObjectId(value["city_id"])},
                              {
                                  "$pull": {f"buildings": b}
                              })
        ...
        b = next((b for b in city["buildings"] if b["id"] == value["building_id"]), None)
        if b:
            db.cities.update_one({"_id": ObjectId(value["city_id"]), "buildings.id": b["id"]},
                             {
                                 "$inc": {f"buildings.$.amount": 1}
                             })
        else:
            building = {"id": value["building_id"], "amount":1}
            db.cities.update_one({"_id": ObjectId(value["city_id"])},
                                 {
                                     "$push": {f"buildings": building}
                                 })
    elif effect == "change_population":
        db.cities.update_one({"_id": ObjectId(value["city_id"])},
                             {
                                 "$inc": {f"population": value["population"]}
                             })
