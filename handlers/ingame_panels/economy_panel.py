...
def open_economy_panel(user, chat_id, message_id):
    ...
    text = (f"{get_date_move(player)}"
            ...
            "\n\n"
            f"Доходы: {get_total_income(player):.2f} монет в ход\n"
            f"Расходы: {get_total_spending(player):.2f} монет в ход \n"
            f"Баланс: {get_total_income(player) - get_total_spending(player):.2f} монет в ход"
            "\n\n"
...
def open_income_panel(user, chat_id, message_id):
    ...
    text = (f"{get_date_move(player)}"
            ...
            f"Общий доход: {get_total_income(player):.2f}"
            f"\n\n"
...
def open_spending_panel(user, chat_id, message_id):
    ...
    text = (f"{get_date_move(player)}"
            ...
            "\n\n"
            f"Общие расходы: {get_total_spending(player):.2f}")
    bot.edit_message_text(
        text,
        chat_id=chat_id,
        message_id=message_id,
